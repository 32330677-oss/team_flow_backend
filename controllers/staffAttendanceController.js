...
exports.runBulkAttendance = async (req, res, mode) => {
    ...
    // ملاحظة: أصبحت هاي العملية All-or-Nothing بالكامل. أي فشل بأي عامل
    // ضمن الدفعة (منطقي أو تقني) بيعمل rollback لكامل الـ transaction،
    // وما بينضاف ولا سجل حضور واحد لأي عامل بالدفعة.
    ...
    try {
        ...
        for (const workerId of workerIds) {
            ...
            if (mode === 'checkin') {
                ...
                if (rows.length > 0) {
                    ...
                    if (!['Absent', 'Sick', 'Vacation', 'Holiday'].includes(existing.attendance_status)) {
                        failedWorker = { worker_id: workerId, message: 'Worker already has an attendance record.' };
                        throw new AppError(`Bulk check-in aborted: worker ${workerId} already has an attendance record. No changes were saved.`);
                    }
                    ...
                    if (updated.affectedRows !== 1) {
                        ...
                        throw new AppError(`Bulk check-in aborted: attendance for worker ${workerId} changed by another request. No changes were saved.`);
                    }
        ...
        return res.status(200).json({ status: responseStatus, successful: workerIds, failed: [] });
    } catch (error) {
